...
private:
    ...
    void _checSeq();
    VGMissionPlan *_parseFlyRoute(const das::proto::OperationDescription &od, DescribeMap &dsc);
    void _parseOperationRoute(const das::proto::OperationRoute &route);
    void _safeDeleteBoundary(const VGLandBoundary &bdr);
    void _safeDeleteFlyRoute(const VGMissionPlan &fr);
    ...
    das::proto::OperationPlan *_transOperationPlan(const VGMissionPlan &fr);
    void registerSuc();
    //上传测绘地块
    void uploadSurveyInfo(const VGLandBoundary &info);
    //上传规划
    void uploadPlanningRoute(const VGMissionPlan &fr);
private:
