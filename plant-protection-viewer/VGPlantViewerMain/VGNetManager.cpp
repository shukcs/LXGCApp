...
void VGNetManager::_processOparetionPlan(const QByteArray &byte)
{
    ...
    if (ackRODess.ParseFromArray(byte.data(), byte.size()))
    {
        ...
        {
            DescribeMap dsc;
            if (VGMissionPlan *fr = _parseFlyRoute(ackRODess.ods(i), dsc))
...
void VGNetManager::_safeDeleteFlyRoute(const VGMissionPlan &fr)
{
...
das::proto::OperationPlan *VGNetManager::_transOperationPlan(const VGMissionPlan &fr)
{
...
void VGNetManager::uploadPlanningRoute(const VGMissionPlan &fr)
{
...
void VGNetManager::DeleteItemSafe(const MapAbstractItem *item)
{
    ...
    if (item->ItemType() == MapAbstractItem::Type_MissionInfo)
        _safeDeleteFlyRoute(*(const VGMissionPlan*)item);
}
...
bool VGNetManager::UploadItem(const MapAbstractItem *item)
{
    ...
    if (item->ItemType() == MapAbstractItem::Type_LandBoundary)
        ...
        uploadPlanningRoute(*(const VGMissionPlan*)item);
    else
        return false;
...
VGMissionPlan *VGNetManager::_parseFlyRoute(const das::proto::OperationDescription &od, DescribeMap &dsc)
{
    if (VGMissionPlan *fr = new VGMissionPlan())
    {
        ...
        for (int i = 0; i < op.bdrsafe_size(); ++i)
        {
            ...
            if (ss.bdr() < 0)
                ...
                fr->m_lsOutlineSafe.push_back(VGMissionPlan::EdgeShrinkPair(ss.bdr(), ss.safe()));
        }
