...
VGValueShow {
    ...
    onBoundaryInfoChanged: {
        if (!boundaryInfo)
            return
        addValue(qsTr("Owner"), boundaryInfo.owner)//qsTr("主人")
        addValue(qsTr("Phone No"), boundaryInfo.ownerPhone)//qsTr("联系方式")
        addValue(qsTr("Area"), boundaryInfo.landArea.toFixed(2))//qsTr("亩数")
        addValue(qsTr("Block"), boundaryInfo.countBlocks.toString())//qsTr("障碍物")
        addValue(qsTr("Survey source"), boundaryInfo.surveyDescribe)//"边界点来源"
        addValue(qsTr("Surveyor"), boundaryInfo.user)//"创建人"
        addValue(qsTr("Create time"), boundaryInfo.editTime)//创建时间
        addValue(qsTr("Notes"), boundaryInfo.describe)//备注
