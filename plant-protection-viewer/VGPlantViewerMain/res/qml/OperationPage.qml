...
Page {
    ...
    VGMap{
        ...
        //bDrawCenter:    missionCtrl.visible && missionCtrl.curOp===2
        ...
        VGImage{
            ...
            onImgClicked:   {
                rctRoute.width = parent.width/2-20
                btnShowRctRoute.visible = false
            }
        }
            ...
            Rectangle {
                ...
                VGRouteList{
                    ...
                    onCreateClick:  {
                        ...
                        else if(tp === MapAbstractItem.Type_WayPointPlan)
                ...
                VGMission {
                    ...
                    onStartMission: {
                        var v = vgTipCom.createObject(root)
                        if (curPlant && (curPlant.restrain || plantManager.sps)) {
							if (curPlant.restrain)
								v.strTip = qsTr("Do u let Uav excute mission?")//"是否执行作业任务？"
							else	
								v.strTip = qsTr("Do u let Uav no rtk excute mission?")//"是否单点执行作业任务？"
                            v.idTip = 2
                        }
                        else {
                            v.strTip = qsTr("Position of Uav is not prepair, do u set param permision arming?")//"飞机未就绪，是否设置参数允许解锁？"
                            v.idTip = 4
                        }
                        vgMainPage.curQmlPage = v
                    }
        ...
        Column{
            id: rowBlock
            anchors {top: colPlant.bottom; topMargin: 15; right: parent.right; rightMargin: 60}
            spacing:                0
            z:                      200
            Repeater{
                model: mapManager.getSpecItems(MapAbstractItem.Type_Block)
                delegate: Rectangle{
                    ...
                    Text{
                        ...
                        color: "red"
                        elide: Text.ElideLeft
                        text:   qsTr("blocks") + object.id + qsTr(":angle:") + object.angle.toFixed(2)
                                + qsTr("degree;distance:") + object.dis.toFixed(2) + qsTr("M")
                    }
                }
            }
        }
