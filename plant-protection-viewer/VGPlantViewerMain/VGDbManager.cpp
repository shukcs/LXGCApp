...
void VGDbManager::_saveRoute(VGMissionPlan &route)
{
...
void VGDbManager::_updateRoute(VGMissionPlan &route)
{
    DescribeMap dsc;
...
void VGDbManager::_processRoute(QSqlQuery &query)
{
    ...
    while (query.next())
    {
        VGMissionPlan *infoDes = new VGMissionPlan(this);
        DescribeMap dsc;
...
void VGDbManager::onUpdateItem(const MapAbstractItem *item)
{
    ...
    if (item->ItemType() == MapAbstractItem::Type_MissionInfo)
        _updateRoute(*(VGMissionPlan*)item);
}
