...
VGVehicleMission::VGVehicleMission(VGMissionPlan *fr) : MapAbstractItem(fr), m_bExecute(false), m_hasSuspend(false), m_msSuspend(false), m_supportEnter(NULL)
...
VGVehicleMission *VGVehicleMission::fromInfo(const QString &info)
{
    ...
    VGMissionPlan *rtTmp = NULL;
    for (VGMissionPlan *rt : qvgApp->landManager()->FlyRoutes())
    {
        DescribeMap tmp;
...
void VGVehicleMission::_generateMission(VGMissionPlan *fr)
{
