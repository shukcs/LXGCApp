...
void VGVehicle::_sendMavCommandAgain()
{
    ...
    MavCommandQueueEntry_t& cmd = m_mavCommandQueue[0];
    if (m_mavCommandRetryCount++ > MaxRetry)
    {
        emit mavCommandResult(cmd.command, false);
    ...
    float *p = cmd.rgParam;
    if (cmd.bInt)
        VGMavLinkCode::EncodeCommandInt(msg, cmd.component, cmd.command,
            qvgApp->mavLink(), priorityLink()->getMavlinkChannel(), m_sysID,
            p[0], p[1], p[2], p[3], cmd.x, cmd.y, cmd.z);
    else
        VGMavLinkCode::EncodeCommandSt(msg, cmd.component, cmd.command,
            qvgApp->mavLink(), priorityLink()->getMavlinkChannel(), m_sysID,
...
void VGVehicle::sendMavCommandInt(MAV_CMD command, float param1, float param2, float param3, float param4, int x, int y, float z)
{
    MavCommandQueueEntry_t entry;
