...
VGVehicleMission *VGPlantInformation::beginOperation(VGMissionPlan *fr)
{
    m_prepareVm = NULL;
    if (fr)
    {
...
void VGPlantInformation::TestMotor(int idx, int s, int pcn, int count)
{
	if (idx >= 8)
		return;

    sendCommandInt(MAV_CMD_DO_MOTOR_TEST, idx, MOTOR_TEST_THROTTLE_PERCENT, pcn, s, count);
}
...
void VGPlantInformation::sendCommandInt(int cmd, float f1, float f2, float f3, float f4, int x, int y, float z)
{
    if (m_bDirectLink)
    {
        if (VGVehicle *v = qvgApp->toolbox()->activeVehicle())
            v->sendMavCommand((MAV_CMD)cmd, f1, f2, f3, f4, x, y, z);
    }
    else if (VGNetManager *nm = qvgApp->netManager())
    {
        mavlink_message_t msg;
        VGMavLinkCode::EncodeCommandInt(msg, 0, (MAV_CMD)cmd, NULL, 0, 0, f1, f2, f3, f4, x, y, z);
        nm->SendControlUav(m_planeId, msg);
    }
}
