...
void VGCoordinate::transCoor2XY(const QGeoCoordinate &home, const QGeoCoordinate &coor, double &x, double &y)
{
    ...
    x = qSin(M_PI * angle / 180) * distance;
    y = qCos(M_PI * angle / 180) * distance;
}
...
void VGCoordinate::remove()
{
    deleteLater();
}

QGeoCoordinate VGCoordinate::MidOf(const VGCoordinate &coor) const
{
    return getMid(m_coordinate, coor.m_coordinate);
}
