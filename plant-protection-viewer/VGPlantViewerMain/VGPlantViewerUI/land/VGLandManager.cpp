...
, m_landCur(nullptr), m_connectState(-1)
, m_landSurvey(nullptr), m_routeCur(nullptr)
...
void VGLandManager::sltSurveyError(int errorInfo)
{
    ...
    {
    case 1000:
        strError = tr("Open GPS denied");//"无打开GPS的权限"
        break;
    case 1001:
        strError = tr("GPS unserver");//"GPS定位服务未开启"
        break;
    case 1002:
        strError = tr("Open GPS fail");//"打开GPS设备失败"
        break;
    case 1003:
        strError = tr("Open GPS success");//"打开GPS成功"
        break;
    case 1004:
        strError = tr("Open GPS timeout");//"打开GPS设备超时");
        break;
    case 1005:
        strError = tr("No GPS is valid");//"无可用GPS设备"
        break;
    case 1006:
        strError = tr("Open GPS fail,used or other");//"打开GPS设备失败，被占用或者其它错误"
        break;
    case 1007:
        strError = tr("Open position device timeout");//"打开定位设备超时"
        break;
    case 1008:
        strError = tr("Open position device channel error");//"打开定位设备通道错误"
        break;
    case 1009:
        strError = tr("Close position device error");//"关闭定位设备错误"
        break;
    case 1010:
        strError = tr("No survey link");//"没有连接测绘设备"
        break;
    default:
        return;
    }
...
const QList<VGMissionPlan*> & VGLandManager::FlyRoutes() const
{
    return m_lstRoute;
}

const QList<VGWayPointPlan*> & VGLandManager::WayPointPlans() const
{
    return m_wpPlans;
}
...
VGMissionPlan *VGLandManager::preparePlanRoute(VGLandBoundary *bdy)
{
    if (!m_flyRoutePlan)
    {
        m_flyRoutePlan = _getPlanFlyRoute(bdy);
        PreparePlanRoute(m_flyRoutePlan);
    }
...
void VGLandManager::quitOperation()
{
    ...
    MapAbstractItem *item = mm ? mm->GetSelecedItem(MapAbstractItem::Type_MissionInfo) : NULL;
    if (VGMissionPlan *rt = dynamic_cast<VGMissionPlan*>(item))
...
VGMissionPlan *VGLandManager::GetCurFlyRoute() const
{
    return m_routeCur;
}
...
QList<VGMissionPlan*> VGLandManager::GetRoutesOfBoundary(VGLandBoundary *bdr)
{
    QList<VGMissionPlan*> ret;
    if (bdr)
    {
        foreach(VGMissionPlan *bdy, m_lstRoute)
        {
...
int VGLandManager::Indexof(VGMissionPlan *rt)
{
...
VGLandBoundary *VGLandManager::getVGLandBoundaryById(const QString &id)
{
    foreach (VGLandBoundary *bdr, m_lstBoundary)
    {
        if(bdr->GetBoundaryId() == id)
            return bdr;
    }
    return NULL;
}
...
VGMissionPlan *VGLandManager::Clone(VGMissionPlan *item, VGLandBoundary *p)
{
    if (!item)
        return NULL;

    if (VGMissionPlan *ft = new VGMissionPlan(*(VGMissionPlan*)item))
    {
        ft->setBelongBoundary(p);
        connect(ft, &VGMissionPlan::selectedChanged, this, &VGLandManager::onSelectedChanged);
        connect(ft, &VGMissionPlan::destroyed, this, &VGLandManager::onChildDestroyed);       
...
void VGLandManager::onChildDestroyed()
{
    ...
    m_lstRoute.removeAll((VGMissionPlan*)obj);
    m_wpPlans.removeAll((VGWayPointPlan*)obj);
}
...
{
    foreach (VGMissionPlan *rt, m_lstRoute)
    {
        DescribeMap tmp;
...
VGMissionPlan * VGLandManager::_getPlanFlyRoute(VGLandBoundary *bdy)
{
    if (VGMissionPlan *tmp = new VGMissionPlan(bdy))
    {
        connect(tmp, &VGMissionPlan::planRouteFinished, this, &VGLandManager::onPlanRouteFinished);
        connect(tmp, &VGMissionPlan::destroyed, this, &VGLandManager::onChildDestroyed);
        connect(tmp, &VGMissionPlan::selectedChanged, this, &VGLandManager::onSelectedChanged);
        return tmp;
    }
...
void VGLandManager::onQueryItem(MapAbstractItem *item, const DescribeMap &dsc)
{
    ...
    if (tp != MapAbstractItem::Type_LandInfo && tp != MapAbstractItem::Type_LandBoundary && tp != MapAbstractItem::Type_MissionInfo)
        return;
    ...
    if (!findItem(tmp))
    {
        ...
        else if (tp == MapAbstractItem::Type_MissionInfo)
            Clone((VGMissionPlan*)item, qobject_cast<VGLandBoundary*>(item->FindParentByProperty(dsc)));
    }
...
bool VGLandManager::appendRoute(VGMissionPlan *fr)
{
    if (!fr)
        return false;
    ...
    if(!strId.isEmpty())
    {
        for (VGMissionPlan* item : m_lstRoute)
        {
...
void VGLandManager::PreparePlanRoute(VGMissionPlan *fr)
{
    if (fr)
    {
        connect(s_planningWorker, &VGPlanningWorker::planFinished, fr, &VGMissionPlan::showPlanningResult, Qt::UniqueConnection);
        s_planningWorker->BoundaryPlan(fr, fr->m_homePoint->GetCoordinate());
    }
}
