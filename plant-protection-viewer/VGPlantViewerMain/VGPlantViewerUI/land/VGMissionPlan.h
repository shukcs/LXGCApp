...
class VGMissionPlan : public SingleTriggerItem<MapAbstractItem>
{
    Q_OBJECT
    Q_PROPERTY(bool planning READ isPlanning NOTIFY planningChanged)
    ...
    Q_PROPERTY(QString planTime READ GetTime NOTIFY planTimeChanged)
    ...
    Q_PROPERTY(bool valide READ IsValide NOTIFY valideChanged)
...
public:
    explicit VGMissionPlan(QObject *info = NULL);
    VGMissionPlan(const VGMissionPlan &oth);
    ~VGMissionPlan();
    ...
    VGVehicleMission *beginOperation();
    ...
    int CountRidges()const;
    float GetAcreage()const;
public slots:
    void showPlanningResult(const VGRoute &result, VGMissionPlan *rt);
protected:
...
private:
    friend class VGDbManager;
    friend class VGNetManager;
    friend class VGLandManager;
