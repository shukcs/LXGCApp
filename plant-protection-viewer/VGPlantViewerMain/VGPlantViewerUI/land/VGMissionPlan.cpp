...
//VGMissionPlan
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VGMissionPlan::VGMissionPlan(QObject *parent) : SingleTriggerItem<MapAbstractItem>(qobject_cast<VGLandBoundary*>(parent))
...
, m_stPlan(St_Idle), m_homePoint(new VGCoordinate()), m_time(0)
, m_bUploaded(false), m_route(NULL), m_fBlockSafeDis(.5)
, m_fAngle(0), m_fPrise(10), m_olMission(NULL),m_ridgeOper(0)
...
, m_bSingleShrink(false), m_fSafeDis(.5)
{
    ...
    connect(this, &VGMissionPlan::singleShrinkChanged, this, &VGMissionPlan::onSingleShrinkChanged);
}

VGMissionPlan::VGMissionPlan(const VGMissionPlan &oth) : SingleTriggerItem<MapAbstractItem>(oth)
...
, m_bUploaded(oth.m_bUploaded), m_strDescribe(oth.m_strDescribe), m_ridgeOper(0)
, m_stPlan(oth.m_stPlan), m_bSaveLocal(oth.m_bSaveLocal), m_bAnti(oth.m_bAnti)
...
, m_fSafeDis(oth.m_fSafeDis), m_bSingleShrink(oth.m_bSingleShrink)
{
    if (oth.m_route && (m_route = new VGLandPolyline(*oth.m_route)))
    {
        ...
        {
            int tp = itr->GetId();
            if (VGCoordinate::RouteBegin==tp || VGCoordinate::RouteOperate==tp)
                m_ridgeOper++;
        }
...
VGLandBoundary *VGMissionPlan::GetBelongedBoundary() const
{
    return (VGLandBoundary *)m_parentItem;
}

VGLandInformation *VGMissionPlan::GetBelongedLand() const
{
...
MapAbstractItem::MapItemType VGMissionPlan::ItemType() const
{
    return MapAbstractItem::Type_MissionInfo;
}

void VGMissionPlan::onChildDestroied(QObject *obj)
{
    if (obj == m_olMission)
        m_olMission = NULL;
}

void VGMissionPlan::onSingleShrinkChanged(bool b)
{
    if (b && m_lsSelectEdge.size() < 1)
    {
        ...
        if (plg)
        {
            ...
            for (int i = 0; i < ls.size(); ++i)
            {
                ...
                m_lsSelectEdge << edge;
                connect(edge, &VGSelectEdge::selected, this, &VGMissionPlan::onSelectedEdge);
            }
            m_nCurEdge = 0;
        }
    }
...
void VGMissionPlan::onSelectedEdge(VGSelectEdge *edge)
{
...
void VGMissionPlan::planRoute(bool chgBdr)
{
    if (St_Planning == m_stPlan)
        m_stPlan = St_Wait;
    ...
    if (canPlanRoute())
    {
        ...
        m_stPlan = St_Planning;
        VGLandManager::PreparePlanRoute(this);
    }
}

bool VGMissionPlan::isPlanning() const
{
    return m_stPlan != St_Idle;
}

bool VGMissionPlan::IsValide() const
{
    return !m_bErrorPlan && m_route!=NULL;
}

VGLandPolyline *VGMissionPlan::allMissionRoute() const
{
    return m_route;
}

QString VGMissionPlan::GetBoundaryID() const
{
...
QString VGMissionPlan::GetActId() const
{
    return m_strActId.isEmpty() ? QString::number(m_time) : m_strActId;
}

void VGMissionPlan::SetActId(const QString &id)
{
    m_strActId = id;
    emit actIdchanged(id);
}

QString VGMissionPlan::GetCropper() const
{
    return m_strCropper;
}

void VGMissionPlan::SetCropper(const QString &c)
{
    if (m_strCropper == c)
        return;
...
QString VGMissionPlan::GetPesticide() const
{
    return m_strPesticide;
}

void VGMissionPlan::SetPesticide(const QString &c)
{
    if (m_strPesticide == c)
        return;
...
double VGMissionPlan::GetPrice() const
{
    return m_fPrise;
}

void VGMissionPlan::SetPrice(double f)
{
    if (m_fPrise == f)
        return;
...
double VGMissionPlan::GetBlockSafeDis() const
{
    return m_fBlockSafeDis;
}

void VGMissionPlan::SetBlockSafeDis(double f)
{
...
double VGMissionPlan::GetOutlineSafe() const
{
    return GetEdgeShink(m_nCurEdge);
}

void VGMissionPlan::releaseSafe()
{
    MapAbstractItem::releaseSafe();
}

void VGMissionPlan::SetOutlineSafe(double f)
{
    if (!m_bSingleShrink)
    {
        if (m_fSafeDis == f)
...
float VGMissionPlan::GetAngle() const
{
    return m_fAngle;
}

void VGMissionPlan::SetAngle(float f)
{
    if (m_fAngle == f)
        return;
...
double VGMissionPlan::GetSupportHeight() const
{
    return 0;
}

void VGMissionPlan::SetSupportHeight(double f)
{
    emit supportHeightChanged(f);
    _adjustSupportAttitude(f);
}

bool VGMissionPlan::save(bool bCloud)
{
...
void VGMissionPlan::processSaveReslt(const DescribeMap &result)
{
...
bool VGMissionPlan::GetAnti() const
{
    return m_bAnti;
}

MapAbstractItem *VGMissionPlan::FindParentByProperty(const DescribeMap &dsc) const
{
...
void VGMissionPlan::SetAnti(bool b)
{
    if (m_bAnti == b)
        return;
    m_bAnti = b;
...
void VGMissionPlan::SetSingleShrink(bool b)
{
...
int VGMissionPlan::GetCurEdge() const
{
    return m_nCurEdge;
}

MapAbstractItem *VGMissionPlan::Clone(QObject *parent) const
{
    if (VGMissionPlan *ret = new VGMissionPlan(*this))
    {
...
QString VGMissionPlan::GetTime() const
{
    return VGMainPage::GetTimeString(m_time);
}

qint64 VGMissionPlan::GetMsTime() const
{
    return m_time;
}

void VGMissionPlan::showContent(bool b)
{
    if (m_parentItem)
    {
        if (b)
...
bool VGMissionPlan::canPlanRoute()const
{
    if (VGLandBoundary *tmp = GetBelongedBoundary())
        return tmp->IsValid() && !m_stPlan;

    return false;
}

VGVehicleMission *VGMissionPlan::beginOperation()
{
    if (!IsValide())
        return NULL;
    ...
    if (!m_olMission)
    {
        ...
        connect(m_olMission, &QObject::destroyed, this, &VGMissionPlan::onChildDestroied);
    }
...
void VGMissionPlan::showSelectEdge(bool b)
{
...
void VGMissionPlan::setBelongBoundary(VGLandBoundary *bdr)
{
    VGLandBoundary *bdrCur = GetBelongedBoundary();
...
VGVehicleMission *VGMissionPlan::GetVehicleMission() const
{
    return m_olMission;
}

QList<MissionItem*> VGMissionPlan::GetMissionItems() const
{
    if (m_olMission)
...
const QList<MissionItem*> &VGMissionPlan::GetSafeBoudaryItems()
{
...
void VGMissionPlan::SetSelected(bool b)
{
...
QList<double> VGMissionPlan::GetBoundarys() const
{
    if (VGLandBoundary *bdr = GetBelongedBoundary())
...
void VGMissionPlan::_genarateDefualtHome(VGLandBoundary *bdr)
{
...
void VGMissionPlan::_adjustSupportAttitude(double alt)
{
    if (!m_route)
        return;
...
void VGMissionPlan::_adjustOperateAttitude(double alt)
{
    if (!m_route)
        return;
...
void VGMissionPlan::_calculateAnggle(VGLandPolyline *rt)
{
    if (!rt)
        return;
...
void VGMissionPlan::_calculateSafeBoundary()
{
    qDeleteAll(m_safeBdrs);
    m_safeBdrs.clear();
    int seq = 0;
    int id = 0;
    for (const CoordinateList &itrLs : VGLandManager::PlanningWorker()->GenShrinkBoudary(this, m_homePoint->GetCoordinate()))
    {
        int i = 0;
...
double VGMissionPlan::GetEdgeShink(int idx) const
{
    if (m_bSingleShrink)
    {
...
int VGMissionPlan::CountBlock() const
{
    if (VGLandBoundary *bdr = GetBelongedBoundary())
...
int VGMissionPlan::CountRidges() const
{
    return m_ridgeOper;
}

float VGMissionPlan::GetAcreage() const
{
    if (VGLandBoundary *b = GetBelongedBoundary())
        return b->GetLandArea();

    return m_operateVoyage * m_sprinkleWidth * 3 / 2000;
}

QList<int> VGMissionPlan::GetEdgeShinkLs() const
{
    QList<int> ret;
    if (!m_bSingleShrink)
    {
...
void VGMissionPlan::showPlanningResult(const VGRoute &result, VGMissionPlan *rt)
{
    if (rt != this)
        return;
    ...
    QGeoCoordinate homeCoor = m_homePoint->GetCoordinate();
    m_ridgeOper = 0;
    if (!m_route)
    {
    ...
    foreach(const VGRoutePoint &point, result.GetRoutePoints())
    {
        ...
        int tp = point.GetType() + VGCoordinate::RouteBegin;
        VGCoordinate *c = new VGCoordinate(coor, tp, m_route);
    ...
    if (m_bErrorPlan)
        qvgApp->SetQmlTip(tr("Can not Plan,please modify parameters!"), true);//"地块无法规划，请尝试修改参数!"
    ...
    {
        m_stPlan = St_Idle;
        emit planningChanged(isPlanning());
    }
}

float VGMissionPlan::GetSprinkleWidth() const
{
    return m_sprinkleWidth;
}

double VGMissionPlan::GetLandArea() const
{
    if (VGLandBoundary *bdr = GetBelongedBoundary())
...
QString VGMissionPlan::GetUser() const
{
    if (VGLandBoundary *bdr = GetBelongedBoundary())
        return bdr->GetUser();
...
QString VGMissionPlan::_bdrSafeDis2Str() const
{
...
void VGMissionPlan::_parseSafeDis(const QString &str)
{
...
QString VGMissionPlan::GetInfoString() const
{
...
void VGMissionPlan::ParseInfoString(const QString &str)
{
...
void VGMissionPlan::SetSprinkleWidth(float f)
{
...
float VGMissionPlan::GetSprinkleCover() const
{
    return m_sprinkleCover;
}

void VGMissionPlan::SetSprinkleCover(double f)
{
...
float VGMissionPlan::GetOperationHeight() const
{
    return m_operationHeight;
}

void VGMissionPlan::SetOperationHeight(float f)
{
...
float VGMissionPlan::GetMedPerAcre() const
{
    return m_medPerAcre;
}

void VGMissionPlan::SetMedPerAcre(float f)
{
    m_medPerAcre = f;
}

double VGMissionPlan::GetMaxRoute() const
{
    return m_maxRoute;
}

void VGMissionPlan::SetMaxRoute(double f)
{
    if (f == m_maxRoute)
        return;
...
QString VGMissionPlan::GetDescribe() const
{
    return m_strDescribe;
}

void VGMissionPlan::SetDescribe(const QString &des)
{
...
QString VGMissionPlan::GetLandName() const
{
...
double VGMissionPlan::GetTotalVoyage() const
{
    return m_totalVoyage;
}

double VGMissionPlan::GetTransVoyage() const
{
    return m_transVoyage;
}

double VGMissionPlan::GetOperateVoyage() const
{
...
bool VGMissionPlan::IsSaveLocal() const
{
	return m_bSaveLocal;
}

bool VGMissionPlan::GetSingleShrink() const
{
    return m_bSingleShrink;
}

void VGMissionPlan::SetSaveLocal(bool bSl)
{
	if (m_bSaveLocal == bSl)
		return;
	m_bSaveLocal = bSl;
...
bool VGMissionPlan::IsUpLoaded() const
{
	return m_bUploaded;
}

void VGMissionPlan::SetUploaded(bool bUpload)
{
...
bool VGMissionPlan::operator==(const MapAbstractItem &item) const
{
    if (item.ItemType() != ItemType())
        return false;

    const VGMissionPlan &fr = *(VGMissionPlan*)&item;
...
DECLARE_ITEM_FACTORY(VGMissionPlan)
