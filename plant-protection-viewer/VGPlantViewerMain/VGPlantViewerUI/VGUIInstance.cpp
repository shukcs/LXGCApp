...
#include "friends/VGFriendsManager.h"
...
,m_friendsManager(new VGFriendsManager(this)), m_missionHistory(new VGMissionHistory(this))
{
    ...
    qmlRegisterUncreatableType<VGMissionPlan>("VGGroundControl", 1, 0, "VGMissionPlan", "Reference only");
    qmlRegisterUncreatableType<VGWayPointPlan>("VGGroundControl", 1, 0, "VGWayPointPlan", "Reference only");
    ...
    qmlRegisterUncreatableType<VGController>("VGGroundControl", 1, 0, "VGController", "Reference only");

    qmlRegisterUncreatableType<VGFriend>("VGGroundControl", 1, 0, "VGFriend", "Reference only");
    qmlRegisterUncreatableType<VGGSMessage>("VGGroundControl", 1, 0, "VGGSMessage", "Reference only");
    qmlRegisterUncreatableType<VGMissionHistory>("VGGroundControl", 1, 0, "VGMissionHistory", "Reference only");

    qRegisterMetaType<QList<double>>("QList<double>");

    connect(m_logDataObject, &VGLogDataObject::updateLogList, this, &VGUIInstance::sltUpdateLogList);
...
VGUIInstance::~VGUIInstance()
{
    ...
    m_vgMainPage = NULL;
    delete m_vgNetManager;
    m_vgNetManager = NULL;
    delete m_surveyMonitor;
    m_surveyMonitor = NULL;
    delete m_landManager;
    m_landManager = NULL;

    delete m_logDataObject;
    m_logDataObject = NULL;
    delete m_plantManager;
    m_plantManager = NULL;

    delete m_friendsManager;
    m_friendsManager = NULL;
    delete m_missionHistory;
    m_missionHistory = NULL;
}
...
void VGUIInstance::createQml()
{
    ...
    m_qmlEngine->rootContext()->setContextProperty("friendsManager", m_friendsManager);
    ...
    m_friendsManager->InitialNetWork(m_vgNetManager);
    m_missionHistory->InitialNet(m_vgNetManager);
    if (m_landManager)
    {
        m_landManager->uploadLandInfo(MapAbstractItem::Type_LandInfo);
        m_landManager->uploadLandInfo(MapAbstractItem::Type_LandBoundary);
        m_landManager->uploadLandInfo(MapAbstractItem::Type_MissionInfo);
    }
}
...
VGFriendsManager *VGUIInstance::friendsManager() const
{
    return m_friendsManager;
}


VGMissionHistory * VGUIInstance::missionHistory() const
{
